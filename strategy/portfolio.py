...
def make_strategy_portfolio(df: pd.DataFrame, logger=None, start_date=None, end_date=None):
    ...
    def solve_date(d: date):
        ...
        for now in times:
            ...
            for (t, w) in weights.items():
                ...
                pr_df = prices_dfs[t]
                price_now = pr_df[pr_df[:, 0] <= now][-1]
                price_next = pr_df[pr_df[:, 0] <= (now + HORIZONT)][-1]

                ret = 0.0
                if w < 0:
                    # ret = price_next['ask_price'] / price_now['bid_price']
                    ret = price_next[1] / price_now[2]
                else:
    ...
    annual = avg_profit * DAYS_PER_YEAR
    log(f"average profit --- {avg_profit} --- std: {std}", logger=logger)
    log(f"sum profit --- {sum_profit}", logger=logger)
    log(f"annualized profit --- {annual}", logger=logger)
